...
exports.changeTeacherToAdmin = functions.region(regionOpts).runWith(runtimeOpts).https.onCall(async (data, context) => {

  if (!context.auth.token.isAdmin) return
  try {
    var _ = await admin.auth().setCustomUserClaims(data.uid, data.role)
    return db.collection("teacherData").doc(data.uid).update({role:data.role})
  }
  catch (error) {
    console.log(error)
  }
...
exports.changeEmployeeToAdmin = functions.region(regionOpts).runWith(runtimeOpts).https.onCall(async (data, context) => {

  if (!context.auth.token.isAdmin) return
  try {
    var _ = await admin.auth().setCustomUserClaims(data.uid, data.role)
    return db.collection("employeeData").doc(data.uid).update({role:data.role})
  }
  catch (error) {
    console.log(error)
  }
